...
ObjFile_novt::ObjFile_novt(const char* name){
	fn = name;
}

void ObjFile_novt::get_vertices(float** V){
  char str[1000];
  float f1, f2, f3;
  std::string s = "a";
  FILE * myObject;
...
void ObjFile_novt::get_normals(float ** N){
  char str[1000];
  float f1, f2, f3;
  std::string s = "a";
  FILE * myObject;
...
void ObjFile_novt::get_face_data(int** face_vertex, int**face_normals){
  ...
  do{    
    s = str;
    k_vf=k_vf+1;
    t =fscanf(myObject, "%s %i %c %c %i %i %c %c %i %i %c %c %i", str, &i1, &c1, &c2, &i3, &i4, &c3,  &c4, &i6, &i7, &c5, &c6, &i9);
  }while(t!=EOF);
...
int ObjFile_novt::get_number_of_faces(void){
  return number_of_faces;
}
